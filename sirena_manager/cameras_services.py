...
def _parse_formats_ext(device: str) -> list:
    """Розбирає вивід `list_formats()` у список (fourcc, width, height, fps).
    Stepwise-діапазони (CSI без реального негоційованого режиму) сюди не
    потрапляють — у них немає супровідного "(N fps)" рядка, який тут
    обов'язковий для запису режиму."""
    modes = []
    fourcc = None
    size = None
...
def _usable_modes(device: str) -> list:
    ...
    орієнтується supervisor.set_camera(), щоб не перемикати на камеру чи
    роздільність, яка одразу зверне пайплайн."""
    out = []
    try:
...
def _has_real_formats(device_node: str) -> bool:
    ...
    вище (реальна відсутність узгодженого Discrete+fps режиму)."""
    try:
