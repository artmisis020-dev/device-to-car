...
@require_login
def api_my_devices():
    devices = repository.list_devices_for_owner(g.user["id"])
...
@device_api_bp.route("/api/devices/<device_id>/claim_code", methods=["POST"])
@require_admin
def api_generate_claim_code(device_id):
    payload, status = claim_service.generate_claim_code(device_id)
    return jsonify(payload), status


@device_api_bp.route("/api/devices/<device_id>/owner", methods=["POST"])
@require_admin
def api_reassign_owner(device_id):
    payload, status = claim_service.reassign_owner(device_id, json_body().get("user_id"))
    return jsonify(payload), status
