...
ВЕНДОРЕНА КОПІЯ з additional_modules/pixel_tracking/tracker.py (той самий
принцип ізоляції, що вже застосований для env_file.py в цьому проєкті —
кожен top-level модуль деплоїться незалежно, без крос-імпортів між
video_module і additional_modules). Джерело правди/тести — там; цю копію
оновлювати вручну при зміні алгоритму.
